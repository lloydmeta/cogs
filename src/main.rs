...
fn main() {
    ...
    let sub_key_help = format!("Azure subscription key. Defaults to environment variable {} if set.", AZURE_SUBSCRIPTION_KEY);
    let v = version();
    ...
    let sub_str = matches
        .value_of(SUBSCRIPTION_KEY).map(|s| s)
        .or(sub_from_str);

    match (matches.value_of(FROM_KEY), matches.value_of(TO_KEY), sub_str) {
        (Some(from), Some(to), Some(sub)) => {
            let (mut core, engine) = build_engine(SubscriptionKey::new(sub));
...
fn build_engine(sub_key: SubscriptionKey) -> (tokio_core::reactor::Core, Engine<hyper_tls::HttpsConnector>) {
